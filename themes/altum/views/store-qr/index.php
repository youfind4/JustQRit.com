    ...
    <nav aria-label="breadcrumb">
        <small>
            <ol class="custom-breadcrumbs">
                ...
                <li class="active" aria-current="page"><?= $this->language->store_qr->breadcrumb ?></li>
            </ol>
        </small>
    </nav>

    <h1 class="h4 text-truncate mr-3"><?= sprintf($this->language->store_qr->header, $data->store->name) ?></h1>
    <p>
        ...
        <div class="col-12 col-lg-6 mb-4 mb-lg-0 d-print-none">
            <div class="card">
                <div class="card-header bg-gray-50 border-0 font-weight-bold">
                    <?= $this->language->store_qr->configurator->header ?>
                </div>

                <div class="card-body">

                    <div class="form-group">
                        <label for="foreground_color"><?= $this->language->store_qr->configurator->foreground_color ?></label>
                    ...
                    <div class="form-group">
                        <label for="background_color"><?= $this->language->store_qr->configurator->background_color ?></label>
                    ...
                    <div class="form-group">
                        <label for="corner_radius"><?= $this->language->store_qr->configurator->corner_radius ?></label>
                        <input type="range" id="corner_radius" class="form-control-range" min="0" max="0.5" step="0.1" value="0" />
                    </div>

                    <div class="form-group">
                        <label for="type"><?= $this->language->store_qr->configurator->type ?></label>
                        <select name="type" id="type" class="form-control">
                            <option value="normal" selected="selected"><?= $this->language->store_qr->configurator->type_normal ?></option>
                            <option value="text"><?= $this->language->store_qr->configurator->type_text ?></option>
                            <option value="image"><?= $this->language->store_qr->configurator->type_image ?></option>
                        </select>
                    </div>

                    <div id="type_text" class="d-none">
                        <div class="form-group">
                            <label for="text"><?= $this->language->store_qr->configurator->text ?></label>
                            <input type="text" id="text" name="text" class="form-control" value=":)" />
                        </div>

                        <div class="form-group">
                            <label for="text_color"><?= $this->language->store_qr->configurator->text_color ?></label>
                            <input type="hidden" id="text_color" name="text_color" class="form-control" value="#000000" />
                            <div id="text_color_pickr"></div>
                        </div>

                        <div class="form-group">
                            <label for="text_size"><?= $this->language->store_qr->configurator->text_size ?></label>
                    ...
                    <div id="type_image" class="d-none">
                        <div class="form-group">
                            <label for="image"><?= $this->language->store_qr->configurator->image ?></label>
                            <input id="image" type="file" name="image" accept=".png, .jpg, .jpeg" class="form-control-file" />
                            <img id="image-buffer" src="" class="d-none" />
                        </div>

                        <div class="form-group">
                            <label for="image_size"><?= $this->language->store_qr->configurator->image_size ?></label>
            ...
            <div class="row mb-4">
                <div class="col-12 col-lg-6 mb-3 mb-lg-0">
                    <button type="button" onclick="window.print()" class="btn btn-block btn-outline-secondary d-print-none">
                        <i class="fa fa-fw fa-sm fa-file-pdf"></i> <?= $this->language->store_qr->print ?>
                    </button>
                </div>

                <div class="col-12 col-lg-6">
                    <button id="download" type="button" class="btn btn-block btn-primary d-print-none">
                        <i class="fa fa-fw fa-sm fa-download"></i> <?= $this->language->store_qr->download ?>
                    </button>
                </div>
            </div>

            <p class="text-muted text-center"><?= $this->language->store_qr->subheader ?></p>
        </div>
    </div>
...
<?php ob_start() ?>
<link href="<?= SITE_URL . ASSETS_URL_PATH . 'css/pickr.min.css' ?>" rel="stylesheet" media="screen,print">
<?php \Altum\Event::add_content(ob_get_clean(), 'head') ?>

<?php ob_start() ?>
    <script src="<?= SITE_URL . ASSETS_URL_PATH . 'js/libraries/jquery-qrcode.min.js' ?>"></script>
