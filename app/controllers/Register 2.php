    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                $stmt = Database::$database->prepare("INSERT INTO `users` (`password`, `email`, `email_activation_code`, `name`, `api_key`, `plan_id`, `plan_expiration_date`, `plan_settings`, `language`, `timezone`, `active`, `date`, `ip`, `country`, `last_user_agent`, `total_logins`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param('ssssssssssssssss', $password, $_POST['email'], $email_code, $_POST['name'], $api_key, $plan_id, $plan_expiration_date, $plan_settings, Language::$language, $timezone, $active, \Altum\Date::$date, $ip, $country, $user_agent, $total_logins);
                $stmt->execute();
